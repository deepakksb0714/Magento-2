...
import { addTransaction as onAddTransaction, getEntitiesLocations as onGetEntitiesLocations } from '../../../slices/thunk';
...
const AddTransaction = () => {
  ...
  const dispatch = useDispatch();
   const selectLocationsList = createSelector(
    ...
    // Function to reload entities locations list
    const reloadEntitiesLocations = () => {
      dispatch(onGetEntitiesLocations());
    };
     // Find the default location
    const defaultLocation = entitiesLocationsList.find((location: any) => location.is_default === true);
    // Reload data when the component mounts
    useEffect(() => {
      reloadEntitiesLocations();
    }, []);
...
useEffect(() => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: any) => {
      ...
      try {
        ...
        try {
          const response = await dispatch(onAddTransaction(transformedData));

          if (onAddTransaction.rejected.match(response)) {
  ...
  const handleCombinedSubmit = async () => {
    // Trigger Formik validation before proceeding
    ...
    if (Object.keys(errors).length > 0) {
      ...
      Object.keys(formik.values).forEach(key => {
        if (typeof formik.values[key] === 'object') {
          Object.keys(formik.values[key]).forEach(nestedKey => {
            formik.setFieldTouched(`${key}.${nestedKey}`, true);
          });
        } else {
                              ...
                              >
                                ...
                                {customersWithCertificates
                                  ...
                                  ?.certificates?.filter((cert) => {
                                    ...
                                    return effectiveDate <= today && cert.is_valid === "valid";
                                  })
                                  .map((cert) => (
