...
const EntityLocationTable: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        await dispatch(onGetEntitiesLocations());
      } catch (error) {
      } finally {
        setIsLoading(false);
      }
      ...
      ...shownColumns.map((col) => {
        if (col.accessor === 'location_code') {
          return {
            ...
            isSortable: true,
            Cell: ({ row }: { row: any }) => {
              const locationCode = row.original.location_code;
              const isPrimary = row.original.is_default;
              return (
                <>
                  {locationCode}{' '}
                  {isPrimary && <span className="text-success fw-bold">Primary</span>}
                </>
              );
            }
          };
  ...
  return (
    <>
        ...
        <Col sm={12}>
          <Tab.Container activeKey={activeTab}>
            ...
            >
              <Nav.Item as="li">
                <Nav.Link
                  eventKey="Location List"
                  onClick={() => setActiveTab('Location List')}
                >
                  <i className="las la-th-list me-1"></i>Locations
                </Nav.Link>
              </Nav.Item>
              <Nav.Item as="li">
                <Nav.Link
                  eventKey="Add Location"
                  onClick={() => setActiveTab('Add Location')}
                >
                  <i className="las la-plus-circle me-1"></i>Add Location
                </Nav.Link>
              </Nav.Item>
              <Nav.Item as="li">
                <Nav.Link
                  eventKey="Export Location"
                  onClick={() => setShowExportModal(true)}
                >
                  <i className="las la-lightbulb me-1"></i>Export Locations
                </Nav.Link>
              </Nav.Item>
            </Nav>
          </Tab.Container>
      ...
      {activeTab === 'Add Location' && <AddLocations flag="hide" />}
