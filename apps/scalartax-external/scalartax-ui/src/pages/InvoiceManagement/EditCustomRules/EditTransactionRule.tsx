...
import { Card, Col, Container, Form, Row, Button, FormCheck, InputGroup } from 'react-bootstrap';
import { useFormik } from 'formik';
import * as Yup from 'yup';
import { useNavigate, useLocation, Link } from 'react-router-dom';
import { editTransactionRule as onEditTransactionRule } from '../../../slices/thunk';
...
const EditTransactionRule = () => {
    ...
    document.title = 'EDIT ADVANCED TRANSACTION RULE  | Dashboard';
    ...
    interface Operation {
        ...
        conditions_ids_to_delete: number[]; // Specify type as array of numbers
        allocations: Allocation[];
        allocations_ids_to_delete: number[]; // Specify type as array of numbers
    }

    interface RuleSet {
        id: number,
        conditions: { id: any, field: string; operator: string; values: string[] }[];
        allocations: { locationId: string }[]
        conditions_ids_to_delete: number[];
        allocations_ids_to_delete: number[];
    }
    ...
    const [formData, setFormData] = useState<NewTransactionRule>({
        ...
        allocateTaxOnSingleLine: rowData?.allocate_tax_on_single_line || false,
        operations: [
            {
                id: Date.now(),
                conditions: rowData?.conditions?.map((condition: any) => ({
                    id: condition.id,
                ...
                allocations: rowData?.allocations?.map((allocation: any) => ({
                    id: allocation.id,
                    taxCode: allocation.tax_code || '',
                    percentage: parseFloat(allocation.percentage) || 0,
                })) || [{ taxCode: '', percentage: 0 }],
        ...
        reportings: [
            {
                ...
                allocations: rowData?.allocations?.map((allocation: any) => ({
                    id: allocation.id,
                    address_id: allocation.address?.id || "",
                    addressLine1: allocation.address?.address_line1 || "",
                    addressLine2: allocation.address?.address_line2 || "",
                    city: allocation.address?.city || "",
                    state: allocation.address?.state || "",
                    zipCode: allocation.address?.zip_code || 0,
                    locationId: allocation.location_id || "",
                })) || [
                        {
                            addressLine1: "",
                ...
                conditions: rowData?.conditions?.map((condition: any) => ({
                    id: condition.id,
                    addressTypes: condition.address_types || [],
                })) || [{ addressTypes: ["SHIP FROM"] }],
                allocations_ids_to_delete: [],
                conditions_ids_to_delete: [],
            },
        ],
        multipleAllocations: [
            {
                ...
                allocations: rowData?.allocations?.map((allocation: any) => ({
                    id: allocation?.id || "",
                    country: allocation.address?.country || "",
                    address_id: allocation.address?.id || "",
                    addressLine1: allocation.address?.address_line1 || "",
                    addressLine2: allocation.address?.address_line2 || "",
                    city: allocation.address?.city || "",
                    state: allocation.address?.state || "",
                    zipCode: allocation.address?.zip_code || 0,
                    locationId: allocation.assign_location_code || "",
                    percentage: parseFloat(allocation.percentage) || 0,
                })),
                conditions_ids_to_delete: [],
                allocations_ids_to_delete: []
        ...
        documentTypes: rowData?.document_types || [],
        defaultLocationCode: rowData?.default_location_code || [],
        ignoreRuleOnError: rowData?.ignore_rule_on_error || false,
        inactive: rowData?.inactive || false,
    });
        ...
        onSubmit: (values: any) => {
            ...
            if (ruleType === "marketPlace") {
                consolidatedOperations = ruleSets.map((ruleSet: any) => ({
                    ...
                    conditions_ids_to_delete: ruleSet.conditions_ids_to_delete,
                    allocations_ids_to_delete: ruleSet.allocations_ids_to_delete
                }));
            } else if (ruleType === "reportingAllocation") {
                consolidatedOperations = reportings.map((reporting: any) => ({
                    id: reporting.id,
                    conditions: [], // Reportings might not have conditions
                    allocations: reporting.allocations.map((allocation: any) => ({
                        id: allocation?.id || "",
                        address_id: allocation.address_id || "",
                    ...
                    conditions_ids_to_delete: reporting.conditions_ids_to_delete,
                    allocations_ids_to_delete: reporting.allocations_ids_to_delete
                }));
                ...
                consolidatedOperations = multipleAllocations.map((multipleAllocation: any) => ({
                    ...
                    conditions_ids_to_delete: multipleAllocation.conditions_ids_to_delete,
                    allocations_ids_to_delete: multipleAllocation.allocations_ids_to_delete
                }));
            }
            ...
            dispatch(onEditTransactionRule(data));
    ...
    //operations
    const handleConditionChange = (operationIndex: number, conditionIndex: number, field: string, value: any) => {
        const updatedConditions = [...formik.values.operations[operationIndex].conditions];
        updatedConditions[conditionIndex][field] = value;
        formik.setFieldValue(`operations[${operationIndex}].conditions`, updatedConditions);
    };

    const handleDeleteCondition = (operationIndex: number, conditionIndex: number) => {
        const updatedOperations = [...formik.values.operations];

        const conditionToDelete = updatedOperations[operationIndex].conditions[conditionIndex];
        if (conditionToDelete?.id) {
            updatedOperations[operationIndex].conditions_ids_to_delete.push(conditionToDelete.id);
        }
    ...
    const handleAddCondition = (operationIndex: number) => {
        const newCondition = { field: '', operator: '', values: [''] };
        formik.setFieldValue(`operations[${operationIndex}].conditions`, [
            ...formik.values.operations[operationIndex].conditions,
            newCondition,
        ]);
    };
    ...
    const handleAddOperation = () => {
        ...
        setFormData((prev) => ({
            ...
            operations: [
                ...
                {
                    id: newOperation.id,
                    conditions: newOperation.conditions,
                    allocations: newOperation.allocations,
                    // Include missing properties with appropriate defaults
                    conditions_ids_to_delete: newOperation.conditions_ids_to_delete || [],
                    allocations_ids_to_delete: newOperation.allocations_ids_to_delete || [],
                },
            ],
        }));
    ...
    const handleDeleteOperation = (operationIndex: number) => {
        ...
        setFormData((prev) => ({
            ...
            operations: [...(formData.operations || [])].filter((op) => op.id !== operationIndex),
        }));
    };

    const handleAllocationChange = (operationIndex: number, allocationIndex: number, field: string, value: any) => {
        const updatedAllocations = [...formik.values.operations[operationIndex].allocations];
        updatedAllocations[allocationIndex][field] = field === 'percentage' ? parseFloat(value) : value;
        formik.setFieldValue(`operations[${operationIndex}].allocations`, updatedAllocations);
    };

    const handleDeleteAllocation = (operationIndex: number, allocationIndex: number) => {
        const updatedOperations = [...formik.values.operations];

        // Retrieve the allocation to be deleted
        const allocationToDelete = updatedOperations[operationIndex].allocations[allocationIndex];

        // Add the ID to `allocations_ids_to_delete` if it exists
        if (allocationToDelete?.id) {
            updatedOperations[operationIndex].allocations_ids_to_delete.push(allocationToDelete.id);
        }
    ...
    //ruleset    
    const handleDeleteRuleSet = (ruleSetIndex: number) => {
        const updatedRuleSet = [...formik.values.ruleSets];
        updatedRuleSet.splice(ruleSetIndex, 1);
        formik.setFieldValue('operations', updatedRuleSet);
    };

    const handleAddRuleSet = () => {
        const newRuleSet = {
            id: Date.now(),
            conditions: [{ id: '', field: "", operator: "", values: [""] }],
            conditions_ids_to_delete: [],
    ...
    const handleAddCriteria = (ruleSetIndex: number) => {
        const updatedruleSets = [...(formData.ruleSets || [])];
        updatedruleSets[ruleSetIndex].conditions.push({ id: '', field: '', operator: '', values: [''] });
        setFormData({ ...formData, ruleSets: updatedruleSets });
    };
    ...
    const handleDeleteCriteria = (ruleSetIndex: number, criteriaIndex: number) => {
        ...
        const conditionToDelete = updatedruleSets[ruleSetIndex].conditions[criteriaIndex];
        // Add the ID to `conditions_ids_to_delete` if it exists
        if (conditionToDelete?.id) {
            updatedruleSets[ruleSetIndex].conditions_ids_to_delete.push(conditionToDelete.id);
        }
    ...
    const handleRSAddValue = (ruleSetIndex: number, criteriaIndex: number) => {
        const updatedRuleSets = [...(formik.values.ruleSets)];
        updatedRuleSets[ruleSetIndex].conditions[criteriaIndex].values.push('');
        formik.setFieldValue("operations", updatedRuleSets);
    };
    ...
    const handleAddReportings = () => {
        const newReporting = {
            id: Date.now(),
            allocations: [{ id: "", addressLine1: "", addressLine2: "", city: "", state: "", zipCode: 0, locationId: "" }],
    ...
    const handleDeleteReportings = (reportingIndex: number) => {
        const updatedReportings = [...formik.values.reportings];
        updatedReportings.splice(reportingIndex, 1);
        formik.setFieldValue('reportings', updatedReportings);
    }

    const handleAddAddress = (reportingIndex: number) => {
        const updatedreportings = [...(formData.reportings || [])];
        updatedreportings[reportingIndex]?.allocations.push({ id: "", addressLine1: "", addressLine2: "", city: "", state: "", zipCode: 0, locationId: "", });
    ...
    const handleDeleteAddress = (reportingIndex: number, addressConditionIndex: number) => {
        const updatedreportings = [...(formData.reportings || [])];
        const allocationToDelete = updatedreportings[reportingIndex].allocations[addressConditionIndex];

        // Add the ID to `allocations_ids_to_delete` if it exists
        if (allocationToDelete?.id) {
            updatedreportings[reportingIndex].allocations_ids_to_delete.push(allocationToDelete.id);
        }
        updatedreportings[reportingIndex].allocations.splice(addressConditionIndex, 1);
        setFormData({ ...formData, reportings: updatedreportings });
    }

    const handleAddressChange = (reportingIndex: number, addressconditionIndex: number, field: string, value: string) => {
        const updatedReportings = [...(formData.reportings || [])];
    ...
    const handleMultipleDeleteOperation = (multipleAllocationIndex: number) => {
        const updatedMultipleAllocations = [...formik.values.multipleAllocations];
        updatedMultipleAllocations.splice(multipleAllocationIndex, 1);
        formik.setFieldValue('multipleAllocations', updatedMultipleAllocations);
    };
    ...
    const handleMultipleConditionChange = (
        ...
        if (field === 'values' && Array.isArray(value)) {
            updatedMultipleConditions[multipleConditionIndex].conditions[conditionIndex].values = value;
        } else if (field !== 'values') {
            updatedMultipleConditions[multipleConditionIndex].conditions[conditionIndex][field] = value as string;
        }
    ...
    const handleMultipleAddCondition = (multipleAllocationIndex: number) => {
        const updatedMultipleConditions = [...(formData.multipleAllocations || [])];
        updatedMultipleConditions[multipleAllocationIndex].conditions.push({ id: '', field: '', operator: '', values: [''] });
        setFormData({ ...formData, multipleAllocations: updatedMultipleConditions });
    };
    ...
    const handleMultipleDeleteCondition = (multipleAllocationIndex: number, conditionIndex: number) => {
        ...
        const conditionToDelete = updatedMultipleAllocations[multipleAllocationIndex].conditions[conditionIndex];
        if (conditionToDelete?.id) {
            updatedMultipleAllocations[multipleAllocationIndex].conditions_ids_to_delete.push(conditionToDelete.id);
        }

        // Remove the condition from the array
        updatedMultipleAllocations[multipleAllocationIndex].conditions.splice(conditionIndex, 1);
    ...
    const handleMultipleAddValue = (multipleAllocationIndex: number, conditionIndex: number) => {
        ...
        formik.setFieldValue("operations", updatedMultipleAllocations);
    };
    ...
    const handleMultipleAddAllocation = (multipleAllocationIndex: number) => {
        const updatedAllocations = [...formik.values.multipleAllocations];
        updatedAllocations[multipleAllocationIndex].allocations.push({ country: '', addressLine1: "", addressLine2: "", city: "", state: "", zipCode: 0, percentage: 0 });
        formik.setFieldValue("multipleAllocations", updatedAllocations);
    };
    ...
    const handleMultipleDeleteAllocation = (multipleAllocationIndex: number, allocationIndex: number) => {
        ...
        // Retrieve the allocation to be deleted
        const allocationToDelete = updatedMultipleAllocations[multipleAllocationIndex].allocations[allocationIndex];

        // Add the ID to `allocations_ids_to_delete` if it exists
        if (allocationToDelete?.id) {
            updatedMultipleAllocations[multipleAllocationIndex].allocations_ids_to_delete.push(allocationToDelete.id);
        }
        // Remove the allocation from the array
    ...
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb pageTitle="Edit advanced Transaction rule" title="Edit advanced transaction rule" />
                    <Card>
                        <Card.Body>
                            <Form onSubmit={formik.handleSubmit}>
                                                        ...
                                                        <div className='mt-4'>
                                                            ...
                                                            {operation.allocations.map((allocation, allocationIndex) => (
                                                                <Row className="mb-3 mt-4 d-flex align-items-center" key={allocationIndex}>
                                                                    <Col md={4}>
                                                                        <Form.Group>
                                                                            <Form.Label htmlFor='taxCode'>TAX CODE</Form.Label>
                                                                            <Form.Control
                                                                                id="taxCode"
                                                                                name="taxCode"
                                                                                type="text"
                                                    ...
                                                    <div key={ruleset.id} className="border p-3 mb-4">
                                                        ...
                                                        <div className='mt-4'>
                                                            ...
                                                            <Row className="mb-3 mt-4 d-flex align-items-center">
                                                                ...
                                                                <Col md={4}>
                                                                    <Form.Group>
                                                                        <Form.Label htmlFor="locationCode">LOCATION CODE</Form.Label>
                                                                        <Form.Control
                                ...
                                <div className="d-flex justify-content-start mt-4">
                                    <Button type="submit" variant="primary" className="me-2" >
                                        Update advanced transaction rule
                                    </Button>
                                    <Button
                                        variant="light"
                                        onClick={() => {
...
export default EditTransactionRule;